...
export default function AllAppplicatons() {
    ...
    const categoryFilter = (e: React.ChangeEvent<HTMLSelectElement>) => {
        if (e.target.value === 'All') {
            ...
            const result = applications?.filter(item => item?.visa_type === e.target.value);
            setApplicationList(result || [])
        }
    }
                        ...
                        isLoading ? <AdminLoading /> : isError ? <AdminError /> : !isSuccess ? <></> :
                            <div>
                                <div className="flex justify-end py-6">
                                    <select name="" id="" defaultValue={""} className="outline-0 border-0 bg-transparent dark:text-slate-200" onChange={categoryFilter}>
                                        <option value="" className="dark:bg-boxdark-2">Choose type</option>
                                        {
                                            ['All', 'Tourist', 'Business', 'Student', 'Work', 'Medical', 'Family']?.map((opt, indx) => {
                                                return <option key={opt + indx} value={opt} className="dark:bg-boxdark-2">{opt}</option>
                                            })
                                        }
                                    </select>
                                </div>
                                                ...
                                                <tr key={application?.id}>
                                                    ...
                                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                                        <p className={`text-black dark:text-white  inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${application?.is_approved ? "bg-success text-success" : "bg-danger text-danger"}`}>
                                                            {application?.is_approved ? 'Approved' : 'Not approved'}
                                                        </p>
                                                    </td>
                                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
