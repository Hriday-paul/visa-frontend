...
export const DeleteApplication = React.memo(({ id }: { id: number }) => {
    ...
    useEffect(() => {
        ...
        if (isError) {
            ...
            toast.error(errType?.data?.error || "Application delete failed");
        }
    }, [isSuccess, isError, error]);

    return (
        <Spin spinning={isLoading} size="large" indicator={<ImSpinner8 className="text-4xl animate-spin" />}>
            <Popconfirm
                title="Delete application"
                description="Are you sure want to delete this application"
                onConfirm={deleteFunc}
                okText="Yes"
                cancelText="No"
            >
                <button disabled={isLoading} className="hover:text-primary disabled:cursor-not-allowed">
                    <svg
